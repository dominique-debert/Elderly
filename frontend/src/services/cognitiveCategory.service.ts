...
export const createCognitiveCategory = async (category: { categoryName: string; description?: string; chapterId: number; typeId: number }) => {
  const { data } = await api.post('/categories/cognitive', category);
  return data;
};

export const updateCognitiveCategory = async (id: string, data: { categoryName: string; description?: string }) => {
