...
export const WellnessList = () => {
  ...
  const filteredChapters = Object.entries(groupedWellness ?? {}).filter(
    ([, types]) => {
      ...
      {filteredChapters.map(([chapterName, types], index) => {
        ...
        return (
          <div key={chapterName} className={index !== 0 ? "mt-12" : "mt-6"}>
            ...
            {mode === "list" && (
              <WellnessListView wellnessCategories={filtered} />
            )}
            {mode === "card" && (
              <WellnessCardView wellnessCategories={filtered} />
            )}
            {mode === "table" && (
              <WellnessTableView wellnessCategories={filtered} />
            )}
          </div>
        );
      })}
