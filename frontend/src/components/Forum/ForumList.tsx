...
export const ForumList = () => {
  ...
  const { data: groupedForums, isLoading, isError } = useQuery({
    queryKey: ['forums'],
  ...
  const processedChapters = Object.entries(groupedForums || {}).flatMap(([typeName, chapters]) => {
  ...
  return (
    <div className="w-full p-4">
      ...
      {processedChapters.length === 0 ? (
        ...
        processedChapters.map(({ chapterName, chapterDescription, forums }, index: number) => (
