...
export function ProgramEditModal({
    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          chapterId: String(
            program.chapterId ?? chaptersFormatted[0]?.id ?? ""
          ),
  ...
  if (loading || !form) {
    const chapterName =
      chapters.find((c) => c.id === program.chapterId)?.name ||
      `ID: ${program.chapterId}`;
    const typeName =
      types.find((t) => t.id === program.typeId)?.name ||
      `ID: ${program.typeId}`;
    return (
      <dialog className="modal modal-open" key={program.id}>
        <div className="modal-box">
          <h3 className="font-bold text-lg">
            Modifier la catégorie de programme
          </h3>
