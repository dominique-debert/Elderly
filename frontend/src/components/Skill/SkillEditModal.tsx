...
import { updateSkillCategory } from '@/services/skillCategory.service';
...
export function SkillEditModal({ skill, onClose, onUpdated }: SkillModalProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          chapterId: String(skill.chapterId ?? chaptersFormatted[0]?.id ?? ''),
          typeId: String(skill.typeId ?? typesFormatted[0]?.id ?? ''),
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateSkillCategory(skill.id.toString(), {
  ...
  if (loading || !form) {
    const chapterName = chapters.find((c) => c.id === skill.chapterId)?.name || `ID: ${skill.chapterId}`;
    const typeName = types.find((t) => t.id === skill.typeId)?.name || `ID: ${skill.typeId}`;
    return (
      <dialog className="modal modal-open" key={skill.id}>
        <div className="modal-box">
          <h3 className="font-bold text-lg">Modifier la catégorie de compétence</h3>
          <form className="flex flex-col gap-4 mt-4 w-full">
            <label className="text-sm -mb-2 mt-4">Nom</label>
            <input type="text" value={skill.categoryName} disabled className="input input-bordered w-full bg-gray-100" />
            <label className="text-sm -mb-2 mt-4">Description</label>
            <textarea value={skill.description || ''} disabled className="textarea textarea-bordered w-full bg-gray-100" />
