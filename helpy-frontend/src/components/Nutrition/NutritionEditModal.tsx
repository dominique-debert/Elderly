...
export function NutritionEditModal({ nutrition, onClose, onUpdated }: NutritionModalProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setForm({
          categoryName: nutrition.categoryName,
          description: nutrition.description || '',
          chapterId: String(nutrition.chapterId ?? chaptersFormatted[0]?.id ?? ''),
          typeId: String(nutrition.typeId ?? typesFormatted[0]?.id ?? ''),
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateNutritionCategory(nutrition.id.toString(), {
  ...
  if (loading || !form) {
    const chapterName = chapters.find((c) => c.id === nutrition.chapterId)?.name || `ID: ${nutrition.chapterId}`;
    const typeName = types.find((t) => t.id === nutrition.typeId)?.name || `ID: ${nutrition.typeId}`;
    return (
      <dialog className="modal modal-open" key={nutrition.id}>
        <div className="modal-box">
          <h3 className="font-bold text-lg">Modifier la catégorie de nutrition</h3>
          <form className="flex flex-col gap-4 mt-4 w-full">
            <label className="text-sm -mb-2 mt-4">Nom</label>
            <input type="text" value={nutrition.categoryName} disabled className="input input-bordered w-full bg-gray-100" />
            <label className="text-sm -mb-2 mt-4">Description</label>
            <textarea value={nutrition.description || ''} disabled className="textarea textarea-bordered w-full bg-gray-100" />
