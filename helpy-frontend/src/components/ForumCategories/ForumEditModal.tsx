...
export function ForumEditModal({ forumCategory, onClose, onUpdated }: ForumModalProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          chapterId: String(forumCategory.chapterId ?? chaptersFormatted[0]?.id ?? ''),
          typeId: String(forumCategory.typeId ?? typesFormatted[0]?.id ?? ''),
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateForumCategory(forumCategory.id.toString(), {
  ...
  if (loading || !form) {
    const chapterName = chapters.find((c) => c.id === forumCategory.chapterId)?.name || `ID: ${forumCategory.chapterId}`;
    const typeName = types.find((t) => t.id === forumCategory.typeId)?.name || `ID: ${forumCategory.typeId}`;
    return (
      <dialog className="modal modal-open" key={forumCategory.id}>
        <div className="modal-box">
          ...
          <form className="flex flex-col gap-4 mt-4 w-full">
            ...
            <input type="text" value={forumCategory.categoryName} disabled className="input input-bordered w-full bg-gray-100" />
            <label className="text-sm -mb-2 mt-4">Description</label>
            <textarea value={forumCategory.description || ''} disabled className="textarea textarea-bordered w-full bg-gray-100" />
