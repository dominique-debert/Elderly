...
export function BadgeEditModal({ badgeCategory, onClose, onUpdated }: BadgeModalProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setForm({
          categoryName: badgeCategory.categoryName,
          description: badgeCategory.description || '',
          chapterId: String(badgeCategory.chapterId ?? chaptersFormatted[0]?.id ?? ''),
          typeId: String(badgeCategory.typeId ?? typesFormatted[0]?.id ?? ''),
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateBadgeCategory(badgeCategory.id.toString(), {
  ...
  if (loading || !form) {
    const chapterName = chapters.find((c) => c.id === badgeCategory.chapterId)?.name || `ID: ${badgeCategory.chapterId}`;
    const typeName = types.find((t) => t.id === badgeCategory.typeId)?.name || `ID: ${badgeCategory.typeId}`;
    return (
      <dialog className="modal modal-open" key={badgeCategory.id}>
        <div className="modal-box">
          ...
          <form className="flex flex-col gap-4 mt-4 w-full">
            ...
            <input type="text" value={badgeCategory.categoryName} disabled className="input input-bordered w-full bg-gray-100" />
            <label className="text-sm -mb-2 mt-4">Description</label>
            <textarea value={badgeCategory.description || ''} disabled className="textarea textarea-bordered w-full bg-gray-100" />
