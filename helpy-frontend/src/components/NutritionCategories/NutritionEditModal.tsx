...
type NutritionModalProps = {
  nutritionCategory: ICategory;
  onClose: () => void;
  onUpdated?: () => void;
};

export function NutritionEditModal({ nutritionCategory, onClose, onUpdated }: NutritionModalProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setForm({
          categoryName: nutritionCategory.categoryName,
          description: nutritionCategory.description || '',
          chapterId: String(nutritionCategory.chapterId ?? chaptersFormatted[0]?.id ?? ''),
          typeId: String(nutritionCategory.typeId ?? typesFormatted[0]?.id ?? ''),
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateNutritionCategory(nutritionCategory.id.toString(), {
  ...
  if (loading || !form) {
    const chapterName = chapters.find((c) => c.id === nutritionCategory.chapterId)?.name || `ID: ${nutritionCategory.chapterId}`;
    const typeName = types.find((t) => t.id === nutritionCategory.typeId)?.name || `ID: ${nutritionCategory.typeId}`;
    return (
      <dialog className="modal modal-open" key={nutritionCategory.id}>
        <div className="modal-box">
          ...
          <form className="flex flex-col gap-4 mt-4 w-full">
            ...
            <input type="text" value={nutritionCategory.categoryName} disabled className="input input-bordered w-full bg-gray-100" />
            <label className="text-sm -mb-2 mt-4">Description</label>
            <textarea value={nutritionCategory.description || ''} disabled className="textarea textarea-bordered w-full bg-gray-100" />
