...
export function ProjectEditModal({ project, onClose, onUpdated }: ProjectModalProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          chapterId: String(project.chapterId ?? chaptersFormatted[0]?.id ?? ''),
          typeId: String(project.typeId ?? typesFormatted[0]?.id ?? ''),
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateProjectCategory(project.id.toString(), {
  ...
  if (loading || !form) {
    const chapterName = chapters.find((c) => c.id === project.chapterId)?.name || `ID: ${project.chapterId}`;
    const typeName = types.find((t) => t.id === project.typeId)?.name || `ID: ${project.typeId}`;
    return (
      <dialog className="modal modal-open" key={project.id}>
        <div className="modal-box">
          <h3 className="font-bold text-lg">Modifier la catégorie d'aide</h3>
          <form className="flex flex-col gap-4 mt-4 w-full">
            <label className="text-sm -mb-2 mt-4">Nom</label>
            <input type="text" value={project.categoryName} disabled className="input input-bordered w-full bg-gray-100" />
            <label className="text-sm -mb-2 mt-4">Description</label>
            <textarea value={project.description || ''} disabled className="textarea textarea-bordered w-full bg-gray-100" />
