...
import { SkillListSwitcher } from "./SkillModeSwitcher";
...
export const SkillList = () => {
  ...
  const filteredGrouped = useMemo(() => {
    ...
    Object.entries(groupedSkills).forEach(([typeName, chapters]) => {
      ...
      {Object.entries(filteredGrouped).sort().map(([typeName, chapters]) => (
        Object.entries(chapters).sort().map(([chapterName, categories], index) => (
          <div key={`${typeName}-${chapterName}`} className={index !== 0 ? 'mt-12' : 'mt-6'}>
            ...
            {mode === 'list' && <SkillListView skillCategories={categories} />}
            {mode === 'card' && <SkillCardView skillCategories={categories} />}
            {mode === 'table' && <SkillTableView skillCategories={categories} />}
          </div>
        ))
      ))}
    </div>
