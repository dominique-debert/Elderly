...
export const createMood = async (category: { name: string; description?: string }) => {
...
export const updateMood = async (id: string, data: { name: string; description?: string }) => {
  const response = await api.patch(`/moods/${id}`, data, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
    },
  });
