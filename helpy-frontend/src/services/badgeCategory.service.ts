...
export const createBadgeCategory = async (category: { categoryName: string; description?: string; chapterId: number; typeId: number }) => {
  const { data } = await api.post('/categories/badges', category);
  return data;
};

export const updateBadgeCategory = async (id: string, data: { categoryName: string; description?: string }) => {
