...
export const createNutritionalCategory = async (
  ...
  try {
    const categoryToCreate = await prisma.nutritionalCategory.create({
      data: req.body
    });
...
export const getAllNutritionalCategories = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const categories = await prisma.nutritionalCategory.findMany({
      orderBy: {
        name: 'asc'
      }
    });
...
export const getNutritionalCategoryById = async (
  ...
  try {
    const category = await prisma.nutritionalCategory.findUnique({
      where: { id }
    });
...
export const updateNutritionalCategory = async (
  ...
  try {
    ...
    const categoryToUpdate = await prisma.nutritionalCategory.update({
      data: {
        ...req.body,
        updatedAt: new Date()
      },
