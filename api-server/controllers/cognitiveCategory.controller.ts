...
export const createCognitiveCategory = async (
  ...
  try {
    const categoryToCreate = await prisma.cognitiveCategory.create({
      data: req.body
    });
...
export const getAllCognitiveCategories = async (
  ...
  try {
    const categories = await prisma.cognitiveCategory.findMany({
      orderBy: {
        name: 'asc'
      }
    });
...
export const getCognitiveCategoryById = async (
  ...
  try {
    const category = await prisma.cognitiveCategory.findUnique({
      where: { id }
    });
...
export const updateCognitiveCategory = async (
  ...
  try {
    ...
    const categoryToUpdate = await prisma.cognitiveCategory.update({
      data: {
        ...req.body,
        updatedAt: new Date()
      },
